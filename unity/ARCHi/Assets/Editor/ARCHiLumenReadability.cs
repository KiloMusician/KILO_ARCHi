...
public static class ARCHiLumenReadability
{
    const string AssetPath = "Assets/ARCHiStudies/LumenPearlV1/archi-lumen-pearl-v1.png";
    const string ScenePath = "Assets/Scenes/ARCHiLumenReadabilityV1.unity";
    const string MenuRoot = "ARCHi/Authoring/";
    const int FixtureLayer = 29;
    ...
    static string OutputRoot => Path.GetFullPath(Path.Combine(ProjectRoot, "../../lumen-pearl-v1/unity-validation"));
    ...
    [MenuItem(MenuRoot + "Create Lumen Readability Scene")]
    public static void Create()
    {
        ...
        if (File.Exists(ScenePath)) throw new InvalidOperationException("Presentation scene already exists; use Inspect and Render Lumen Scene.");
        var contract = JsonUtility.FromJson<Contract>(File.ReadAllText("Assets/ARCHiStudies/LumenPearlV1/asset-contract.json"));
        ...
        var camera = new GameObject("Lumen_Readability_Camera").AddComponent<Camera>();
        ...
        for (int row = 0; row < 2; row++)
        {
            ...
            string materialPath = "Assets/ARCHiStudies/LumenPearlV1/" + panel.name + ".mat";
            ...
            {
                float x = (column - 1) * 4;
                var art = new GameObject((light ? "Light_" : "Dark_") + sizes[column] + "px_Lumen");
    ...
    [MenuItem(MenuRoot + "Inspect and Render Lumen Scene")]
    public static void InspectAndRender()
    {
        CheckProject();
        var scene = SceneManager.GetActiveScene();
        if (scene.path != ScenePath || scene.isDirty) throw new InvalidOperationException("Open the saved Lumen presentation scene without unsaved changes.");
        ...
        try {
            ...
            pixels.Apply();
            receipt.render = Path.Combine(OutputRoot, "lumen-readability-unity.png");
        ...
        Debug.Log("ARCHi Lumen presentation rendered: " + receipt.render);
    }
}
